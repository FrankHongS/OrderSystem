window.loadAllOrders = function (name) {
    const order = {};
    const size = 10;
    // current page
    ...
    function displayData(data) {
        ...
        buildOrderTable('.all-orders-container tbody', transformData(order.data));
        if (!isJump) {
            isFirstTime = true;
            count = data.count;
            buildPager();
    ...
    function buildPager() {
        if (!count) {
            $('#all-orders-pager-container').hide();
        } else {
            $('#all-orders-pager-container').show();
        }

        layui.use(['laypage'], () => {
            const laypage = layui.laypage;
            laypage.render({
                elem: 'all-orders-pager-container',
                count: count,
                limit: size,
                curr: page + 1,
                hash: 'page',
    ...
    function bindTableClick() {
        ...
        $('.all-orders-container .delete').on('click', function (e) {
            if (confirm('删除该项')) {
                const index = $(e.target).parent().parent().index();
